...
def run_game():
    ...
    if get_gem_at(game_board, 3, 5) == game_board[3][5] and get_gem_at(game_board, 7, 1) == game_board[7][1] \
            and get_gem_at(game_board, -3, 0) is None and get_gem_at(game_board, 3, -1) is None and \
            get_gem_at(game_board, 3, BOARD_HEIGHT + 3) is None and get_gem_at(game_board, BOARD_HEIGHT, 3) is None \
            and get_gem_at(game_board, 6, BOARD_WIDTH) is None:
    ...
    while True:
        ...
        if clicked_space and not first_selected_gem:
            ...
            matched_gems = get_switched_list(find_matching_gems(get_switched_mat(game_board)))
            ...
            if not can_make_move(get_switched_mat(game_board)):
                game_is_over = True
        ...
        for y in range(BOARD_HEIGHT - 1, -1, -1):  # start from bottom, going up
            if board_copy[x][y] == EMPTY_SPACE:
                ...
                for offset_x, offset_y in ((0, -1), (1, 0), (0, 1), (-1, 0)):
                    ...
                    neighbor_gem = get_gem_at(get_switched_mat(board_copy), y + offset_y, x + offset_x)
...
def get_switched_list(matches_list):
    new_list = []
    for match in matches_list:
        switched_match = [(t[1], t[0]) for t in match]
        new_list.append(switched_match)
    return new_list
