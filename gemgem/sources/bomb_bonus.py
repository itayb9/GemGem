...
def run_game():
    ...
    while True:
        ...
        if clicked_space and not first_selected_gem:
            ...
            bomb_set = []
            # BONUS 1 check if one of the gems is a bomb
            if first_swapping_gem['imageNum'] == LOCATION_BOMB:
                bomb_set = get_local_bomb_gems_to_remove(game_board, second_swapping_gem['x'], second_swapping_gem['y'])
            elif second_swapping_gem['imageNum'] == LOCATION_BOMB:
                bomb_set = get_local_bomb_gems_to_remove(game_board, first_swapping_gem['x'], first_swapping_gem['y'])
            # check if one of the gems is a color bomb
            elif first_swapping_gem['imageNum'] == COLOR_BOMB:
                bomb_set = get_color_bomb_gems_to_remove(game_board, second_swapping_gem['imageNum'])
            elif second_swapping_gem['imageNum'] == COLOR_BOMB:
                bomb_set = get_color_bomb_gems_to_remove(game_board, first_swapping_gem['imageNum'])

            if bomb_set:
                matched_gems.append(bomb_set)
                ...
                while matched_gems:
                    ...
                    for gemSet in matched_gems:
                        ...
                        points.append({'points': score_add,
                                       'x': gemSet[0][0] * GEM_IMAGE_SIZE + X_MARGIN,
                                       'y': gemSet[0][1] * GEM_IMAGE_SIZE + Y_MARGIN})
                    random.choice(GAME_SOUNDS['match']).play()
                    score += score_add

                    # BOMB BONUS : get the new board
                    switched_board_copy = get_switched_board(copy.deepcopy(game_board))
                    game_board = get_switched_board(get_new_board(switched_board_copy, get_switched_matrix(matched_gems)))

                    # BONUS - Put the sleep obstacle
                    if score > SLEEP_OBSTACLE_POINTS_NUM and num_of_sleep_obstacles < 2:
                        game_board[gemSet[0][0]][0] = SLEEP_OBSTACLE
...
def load_images():
    ...
    bomb_image = pygame.transform.smoothscale(bomb_image, (GEM_IMAGE_SIZE, GEM_IMAGE_SIZE))
    gem_images.append(bomb_image)

    color_bomb_image = pygame.image.load('../images/color_bomb.png')
    color_bomb_image = pygame.transform.smoothscale(color_bomb_image, (GEM_IMAGE_SIZE, GEM_IMAGE_SIZE))
    gem_images.append(color_bomb_image)

    return gem_images


# Change the X and Y in the matches list
# Change X and Y in a list
def get_switched_list(list_to_switch):
    return [(t[1], t[0]) for t in list_to_switch]
